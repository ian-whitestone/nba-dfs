...
corrplot.mixed(C1,lower="number",upper='pie',col=col2(10))
```


```{r echo=FALSE, message=FALSE, warning=FALSE}
C2 = cor(team_data[,.(pg_fd,sg_fd,sf_fd,pf_fd,c_fd,team_fd,opp_fd)])
corrplot.mixed(C2,lower="number",upper='circle',col=col2(10))
```
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
C4 = cor(team_data[,.(g_fd,p_fd,opp_g_fd,opp_c_fd)])
corrplot.mixed(C4,lower="number",upper='circle',col=col2(10))
```


--add opposing starters in this?
```{r echo=TRUE, message=FALSE, warning=FALSE, results = 'hide'}
starter_data = dcast(player_data[starter == 1,],gameID + team ~ position, fun.aggregate = mean, value.var = c('fd'))
starter_data = as.data.table(starter_data)
```


```{r echo=FALSE, message=FALSE, warning=FALSE}
C5 = cor(starter_data[is.na(PG) == FALSE & is.na(SG) == FALSE & is.na(SF) == FALSE & is.na(PF) == FALSE & is.na(C) == FALSE,.(PG,SG,SF,PF,C)])
corrplot.mixed(C5,lower="number",upper='circle',col=col2(4))
```

player_data[starter == 1,] %>% dcast(date + team ~ position, value.var = c('fd')) %>% select(C, PF, PG, SF, SG) %>% cor %>% round(2)

starter_data = dcast(player_data[starter == 1,],gameID + team ~ position, fun.aggregate = mean, value.var = c('fd'))
starter_data = as.data.table(starter_data)
cor(starter_data[is.na(PG) == FALSE & is.na(SG) == FALSE & is.na(SF) == FALSE & is.na(PF) == FALSE & is.na(C) == FALSE,.(PG,SG,SF,PF,C)]) %>% round(2)
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
ggplot(player_data[position=='C'],aes(x = opp_rebounds, y = fd)) + 
  geom_point(colour=palette[5]) + theme_dlin() +
