...
player_data[,fd:=1*point+1.2*rebounds+1.5*assists+2*blocks+2*steals-1*turnovers]
...
ggplot(player_data,aes(x = pos_depth, y = minutes)) + geom_point(colour=palette[5]) + 
...
M3=cor(player_data[is.na(minutes_1) == FALSE & is.na(minutes_2) == FALSE & 
                   ...
                   .(fd,minutes_1,minutes_2,minutes_3,minutes_5,minutes_15,
...
ggplot(lebron,aes(reorder(factor(opponent),-mean_fd),mean_fd,
                  colour=factor(season_code))) + geom_point() + 
  theme_dlin() + facet_grid(season_code~.) +
